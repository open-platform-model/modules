// Components defines a single Minecraft Java server workload.
//
//   - One `server-{name}` component (StatefulSet + Service)
//   - Optional `code-server`, `restic-gui`, `rcon-web-admin` components, each
//     scoped to this one server (default off)
//
// The server's Service is annotated for mc-router Kubernetes service-discovery:
//   mc-router.itzg.me/externalServerName = "{name}.{domain}[,{alias}…]"
//   mc-router.itzg.me/defaultServer       = "true"  (when defaultServer)
// The router itself lives in the separate mc_router module.
...
#components: {
	...
	for _srvName, _srvCfg in {"\(#config.name)": #config} {
		...
		"server-\(_srvName)": {
			...
			metadata: {
				...
				// mc-router Kubernetes service-discovery annotations. These land on
				// the generated Service; a router running with IN_KUBE_CLUSTER reads
				// them to auto-register this server (primary hostname + any aliases).
				annotations: {
					"mc-router.itzg.me/externalServerName": strings.Join(
						list.Concat([
							["\(_srvName).\(_domain)"],
							[if _c.aliases != _|_ for _a in _c.aliases {_a}],
						]), ",")
					if _c.defaultServer {
				...
				spec: {
					...
					container: {
						...
						volumeMounts: {
							...
							// Mount for this server — /servers/{name}
							for _srvName, _ in {"\(#config.name)": #config} {
	...
	if #config.resticGui != _|_ {
		if #config.resticGui.enabled {
			...
			// This server's restic repo, if it has one (0- or 1-element list so the
			// downstream `for _, _s in _resticServers` loops are unchanged).
			let _resticServers = [
				for _name, _c in {"\(#config.name)": #config}
