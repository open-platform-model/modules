...
2. **Create the change directory**
   ...
   repo's project-local `module-change` schema (proposal → design → tasks, NO specs artifact).
   ...
   **Then, in the same step, write the change metadata (REPO-LOCAL PATCH, mandatory):**
   ...
   ```yaml
   schema: module-change
   created: <YYYY-MM-DD>
   skip_specs: true
   ```
   Do not create a `specs/` directory inside the change, now or later.

   **Declare the enhancement now, not at archive time (REPO-LOCAL PATCH):** if the change
   implements decisions from an entry in the sibling `enhancements/` repo, write
   `<changeRoot>/enhancement.yaml` while the entry is in context:
   ```yaml
   implements:
     - enhancement: "NNNN"
       decisions: [D1, D2]
       resolves: []
   ```
   `enhancements/schema.cue` `#ChangeDeclaration` validates it (`decisions` are `Dn`, `resolves`
